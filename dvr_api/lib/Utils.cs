...
namespace dvr_api
{
    public class Utils
    {
        ...
        */
        ...
        {
            // if we find an number at a higher position than this then its not an id.
            int maxPosIdCanBe = 2;
            if (!string.IsNullOrEmpty(message))
            {
                string[] arr = message.Split(';', StringSplitOptions.TrimEntries);
                if (arr.Length < 2) { return null; } // if smaller than smallest possible message like: $VIDEO;123456 then return null.
